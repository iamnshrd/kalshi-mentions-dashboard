...
def _build_markdown(market: NormalizedMarket, c: Classification, r: Recommendation) -> str:
    lines = [
        ...
        f"- Yes bid: {market.yes_bid if market.yes_bid is not None else '-'}",
        f"- Yes ask: {market.yes_ask if market.yes_ask is not None else '-'}",
        f"- Last price: {market.last_price if market.last_price is not None else '-'}",
        f"- Volume: {market.volume if market.volume is not None else '-'}",
        "",
        "## Classification",
