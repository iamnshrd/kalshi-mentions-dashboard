...
class KalshiClient:
    ...
    @classmethod
    def _pick_float(cls, raw: dict[str, Any], *keys: str) -> float | None:
        for key in keys:
            if key in raw:
                value = cls._as_float(raw.get(key))
                if value is not None:
                    return value
        return None

    @classmethod
    def _normalize_market(cls, raw: dict[str, Any]) -> NormalizedMarket:
        yes_bid = cls._pick_float(raw, 'yes_bid', 'best_bid')
        yes_ask = cls._pick_float(raw, 'yes_ask', 'best_ask')
        no_bid = cls._pick_float(raw, 'no_bid')
        no_ask = cls._pick_float(raw, 'no_ask')
        yes_price = cls._pick_float(raw, 'yes_price', 'last_yes_price', 'price')
        no_price = cls._pick_float(raw, 'no_price', 'last_no_price')
        last_price = cls._pick_float(raw, 'last_price', 'price')
        volume = cls._pick_float(raw, 'volume', 'volume_24h')
