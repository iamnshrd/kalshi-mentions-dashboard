...
def render_dashboard(output_dir: Path) -> tuple[Path, int]:
    ...
    body = f'''<div class="hero"><h1>Kalshi Mentions Dashboard</h1><p class="muted">Вспомогательный событийный дашборд: помогает быстро проверить контекст, страйки и лайв-фазы, но не заменяет ручное решение трейдера.</p><div class="row"><div class="stat">Событий: <strong>{len(summaries)}</strong></div><div class="stat"><a href="miniapp/index.html">Открыть Telegram Mini App view</a></div><div class="stat"><a href="price-history/index.html">Страница price history / strike chart</a></div></div></div><div class="grid">{''.join(cards) if cards else '<div class="card"><p class="muted">Пока нет event summaries.</p></div>'}</div>'''
